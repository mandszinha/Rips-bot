...
async def verificar(ctx):
    ...
    await ctx.send("📱 Qual é o seu @ no TikTok? (sem o @, ex: `usuario123`)")
    ...
    waiting_msg = await ctx.send(f"🔍 Verificando se **@{username}** segue {TIKTOK_USER}... (pode demorar)")
    ...
    if result is True:
        try:
            ...
            await waiting_msg.edit(content="✅ Você segue, mas não foi possível atribuir o cargo. Verifique permissões.")
    elif result == "private":
        ...
        await waiting_msg.edit(content="❌ Erro ao verificar TikTok. Tente novamente mais tarde.")
    else:
